...
| 0:15–0:45 | Register, then log in | "Registration hashes the password with Argon2id — never stored in plaintext. The session is an httpOnly cookie, so this app's own JavaScript never touches the raw token." | Fill the register form and submit: you land on the login page with an "Account created" message (registering does not sign you in). Log in with the same credentials to reach the dashboard. |
