...
PORT="${DB_PORT:-5432}"  # set DB_PORT when 5432 is taken
...
sed -e "s|^SECRET_KEY=.*|SECRET_KEY=$KEY|" \
    ...
    -e "s|^DATABASE_URL=.*|DATABASE_URL=postgresql+asyncpg://ih_app:$APP_PW@127.0.0.1:$PORT/ih_platform|" \
    -e "s|^MIGRATION_DATABASE_URL=.*|MIGRATION_DATABASE_URL=postgresql+asyncpg://ih_migrator:$MIG_PW@127.0.0.1:$PORT/ih_platform|" \
